...
class AdminControllerTest {
    ...
    void adminPage_returnsAdminViewWithModel() throws Exception {
        when(questionService.findFiltered(any(), any(), any(), any())).thenReturn(List.of());
        ...
        when(questionService.findAllChapters(any())).thenReturn(Collections.emptyList());
    ...
    void importFile_validTxtFile_flashesSuccess() throws Exception {
        MockMultipartFile file = new MockMultipartFile("file", "questions.txt", "text/plain", "#Q\nA\n".getBytes());
        when(questionService.importFromFile(any(), any())).thenReturn(1);

        mockMvc
            .perform(multipart("/admin/import").file(file).with(user(principal)).with(csrf()))
    ...
    void importFile_nonTxtFile_flashesError() throws Exception {
        MockMultipartFile file = new MockMultipartFile("file", "notes.pdf", "application/pdf", "data".getBytes());
