...
public interface QuestionRepository extends JpaRepository<Question, Long> {
    ...
    @Query("SELECT q FROM Question q WHERE q.user = :user AND (q.status IS NULL OR q.status = :unanswered) AND q.sortOrder > :sortOrder ORDER BY q.sortOrder ASC")
    List<Question> findFirstUnansweredAfterSortOrder(@Param("user") AppUser user, @Param("unanswered") QuestionStatus unanswered, @Param("sortOrder") int sortOrder);

    @Query("SELECT q FROM Question q WHERE q.user = :user AND q.status = :status AND q.sortOrder > :sortOrder ORDER BY q.sortOrder ASC")
    List<Question> findFirstByStatusAfterSortOrder(@Param("user") AppUser user, @Param("status") QuestionStatus status, @Param("sortOrder") int sortOrder);
