...
public class QuestionService {
    ...
    public QuestionService(QuestionRepository repo, ChapterRepository chapterRepo, ImportService importService) {
        this.repo = repo;
    ...
    public Optional<Question> findNextUnanswered(AppUser user, Set<Long> chapterIds) {
        if (chapterIds == null || chapterIds.isEmpty()) return findNextUnanswered(user);
        return repo.findFirstUnansweredByChapters(user, QuestionStatus.UNANSWERED, chapterIds).stream().findFirst();
    }

    public Optional<Question> findNextFailed(AppUser user, Set<Long> chapterIds) {
        if (chapterIds == null || chapterIds.isEmpty()) return findNextFailed(user);
        return repo.findFirstByStatusAndUserByChapters(user, QuestionStatus.FAILED, chapterIds).stream().findFirst();
    }

    public Optional<Question> findNextUnansweredAfter(int sortOrder, AppUser user, Set<Long> chapterIds) {
        if (chapterIds == null || chapterIds.isEmpty()) return findNextUnansweredAfter(sortOrder, user);
        return repo
            .findFirstUnansweredAfterSortOrderByChapters(user, QuestionStatus.UNANSWERED, sortOrder, chapterIds)
            .stream()
            .findFirst();
    }

    public Optional<Question> findNextFailedAfter(int sortOrder, AppUser user, Set<Long> chapterIds) {
        if (chapterIds == null || chapterIds.isEmpty()) return findNextFailedAfter(sortOrder, user);
        return repo
            .findFirstByStatusAfterSortOrderByChapters(user, QuestionStatus.FAILED, sortOrder, chapterIds)
            .stream()
            .findFirst();
    }
    ...
    public int importFromFile(MultipartFile file, AppUser user) throws IOException {
        ...
        chapterRepo.findAllByUserOrderBySortOrderAsc(user).forEach(c -> chapterCache.put(c.getName(), c));
        ...
        for (Question q : parsed) {
            ...
            if (chapterName != null && !chapterName.isBlank()) {
                Chapter chapter = chapterCache.computeIfAbsent(chapterName, name -> {
                    Chapter c = new Chapter(name, (int) (chapterCount + chapterCache.size()), user);
    ...
    public List<Question> findFiltered(String text, QuestionStatus statusFilter, String chapterFilter, AppUser user) {
        ...
        final String lowerChapter = (chapterFilter != null && !chapterFilter.isBlank())
        ...
        return all
            ...
            .filter(
                q ->
                    lowerChapter == null ||
                    (q.getChapter() != null && q.getChapter().getName().toLowerCase().contains(lowerChapter))
            )
    ...
    // ── Counts filtered by chapters ──────────────────────────────────────────

    public long countTotalByChapters(AppUser user, Set<Long> chapterIds) {
        if (chapterIds == null || chapterIds.isEmpty()) {
            return countTotal(user);
        }
        List<Question> filtered = repo
            .findAllByUserOrderBySortOrderAsc(user)
            .stream()
            .filter(q -> q.getChapter() != null && chapterIds.contains(q.getChapter().getId()))
            .toList();
        return filtered.size();
    }

    public long countUnansweredByChapters(AppUser user, Set<Long> chapterIds) {
        if (chapterIds == null || chapterIds.isEmpty()) {
            return countUnanswered(user);
        }
        List<Question> filtered = repo
            .findAllByUserOrderBySortOrderAsc(user)
            .stream()
            .filter(q -> q.getStatus() == QuestionStatus.UNANSWERED)
            .filter(q -> q.getChapter() != null && chapterIds.contains(q.getChapter().getId()))
            .toList();
        return filtered.size();
    }

    public long countSuccessByChapters(AppUser user, Set<Long> chapterIds) {
        if (chapterIds == null || chapterIds.isEmpty()) {
            return countSuccess(user);
        }
        List<Question> filtered = repo
            .findAllByUserOrderBySortOrderAsc(user)
            .stream()
            .filter(q -> q.getStatus() == QuestionStatus.SUCCESS)
            .filter(q -> q.getChapter() != null && chapterIds.contains(q.getChapter().getId()))
            .toList();
        return filtered.size();
    }

    public long countFailedByChapters(AppUser user, Set<Long> chapterIds) {
        if (chapterIds == null || chapterIds.isEmpty()) {
            return countFailed(user);
        }
        List<Question> filtered = repo
            .findAllByUserOrderBySortOrderAsc(user)
            .stream()
            .filter(q -> q.getStatus() == QuestionStatus.FAILED)
            .filter(q -> q.getChapter() != null && chapterIds.contains(q.getChapter().getId()))
            .toList();
        return filtered.size();
    }
